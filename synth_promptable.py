...
type_dict_reverse = {0:WaveType.SINE, 1:WaveType.SQUARE, 2:WaveType.SAW, 3:WaveType.NOISE}
...
def main():
    ...
    my_synth = synth(WaveType.SAW, 0.1, WaveType.SINE, 0.5,
                     mix_pct=0.9,
                     adsr=[0.1, 0.1, 0.5, 2],
                     lfo_1=[2, 0],
                     lfo_2=[10, 0])
...
class synth:
    ...
    def assign_params_from_array(self, arr: np.array):
        self.osc_1 = type_dict_reverse[np.argmax(arr[0:4])]
        self.osc_1_sqpct = arr[4] if arr[4] >= 0 and arr[4] <= 1 else 0.5
        self.osc_2 = type_dict_reverse[np.argmax(arr[5:9])]
